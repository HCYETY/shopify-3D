  ...
  class ProductSlider extends HTMLElement {
    ...
    buildItems() {
      this.activeImages = Array.from(
        this.querySelectorAll(".product-images__slide.is-active .product-single__media-image")
      )
...
if (!customElements.get("product-side-drawer")) {
  class ProductSideDrawer extends HTMLElement {
    constructor() {
      super()
      this.init()
    }
    ...
    init() {
      // Get all side drawer links on the page
      const drawerLinks = document.querySelectorAll(".side-drawer-link")

      // Add click event listener to each link
      drawerLinks.forEach((link) => {
        link.addEventListener("click", this.openDrawer.bind(this))
      })
    }
    ...
    openDrawer(event) {
      ...
      // Get the drawer ID from the button's aria-controls attribute
      const drawerId = event.currentTarget.getAttribute("aria-controls")
  ...
  customElements.define("product-side-drawer", ProductSideDrawer)

  // Initialize on page load
  document.addEventListener("DOMContentLoaded", () => {
    new ProductSideDrawer()
  })
}
