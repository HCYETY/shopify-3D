...
if (!customElements.get("theme-header")) {
  class ThemeHeader extends HTMLElement {
    ...
    connectedCallback() {
      ...
      const mobileSidebars = this.querySelectorAll(".mobile-sidebar-menu-item")
      const mobileSidebarsTarget = this.querySelectorAll(".mobile-sidebar-menu-item-link")
      const mobileSidebarsContent = this.querySelectorAll(".mobile-sidebar-menu-item-content")

      mobileSidebarsTarget.forEach((target, index) => {
        target.addEventListener("click", (event) => {
          // event.parentElement
          mobileSidebars.forEach((ms, i) => {
            if (i === index && !ms.classList.contains("active")) {
              ms.classList.add("active")

              mobileSidebarsContent[i].style.height = "auto"

              const h = mobileSidebarsContent[i].offsetHeight
              mobileSidebarsContent[i].style.height = "0"
              mobileSidebarsContent[i].offsetHeight
              mobileSidebarsContent[i].style.height = `${h}px`
            } else {
              ms.classList.remove("active")

              mobileSidebarsContent[i].style.height = "0px"
            }
          })
        })
      })
